...
### 2026-07-28 — PLAN 1 CLOSED: Phase F complete, final review passed, merged to main

- **What**: Taste gate ran four rounds with Joseph. Round 1 -> Task 11a (six build changes; task review found 2 Important, fixed, re-review clean). Round 2 -> Task 11b (eight changes: emit cap, edition merge, cap-key canonicalization, singles+box filters, leaders matcher, shelf ordering; review APPROVED 0 Critical/0 Important). Round 3 -> Task 11c (singles allowlist, We Insist hand-merge). Baked data committed, then the FINAL WHOLE-BRANCH REVIEW ran as three concurrent lenses (correctness/integrity, secrets/robustness, minors-triage/tests) over 39 commits. It found 16 findings incl. 3 Critical; one fix wave in two agents on a strict file partition fixed all of them; the scoped re-review verdicted all 16 plus an addendum ADDRESSED and safe to merge. Merged to main, 347 tests green on the merged result, integrity PASS.
- **Why (a future session can't infer this)**: (1) A LIVE SPOTIFY REFRESH TOKEN was found committed at `.cache` on a PUBLIC repo — it entered at 8d22214 (2026-03-23, ironically the commit that removed the playlist feature), matched no gitignore rule, and was on origin/main. Joseph revoked the app; untracked+ignored in 721fec5. History was NOT rewritten (his call). The June audit had checked tracked files, gh-pages and dist and concluded "not leaked" — it never enumerated tracked DOTFILES. A credential audit must list what git actually tracks. (2) TWO CLAIMS WERE WRONG ON DISK and are now fixed: an album displayed a Discogs rating absorbed from another pressing during the edition merge while its own score was computed without it; and the `sideman` reason fired on non-performer credits, so an album claimed Miles Davis as a sideman on a `Written-By` composer credit. The fetcher had discarded credit ROLES at fetch time, so fixing it needed a re-extraction — free, because the raw HTTP cache (1938 responses) was intact. Sideman reasons fell 21 -> 11; the vindicating catch is "Bill Evans (3)", a TENOR SAXOPHONIST who is not Bill Evans the pianist — Discogs' `(n)` suffix is an entity disambiguator and the old code stripped it before comparing identities. (3) THE HONEST BOUNDARY of the zero-hallucination promise, established by perturbation testing (all 7 reason types caught): the ~559 reason SENTENCES are guaranteed; BADGES, scores, years, titles and all of library.json are NOT. 32 albums carry zero reasons. "Fresh" means re-read from disk, not re-fetch — it catches drift, not wrongness. State this before anyone builds UI on it. (4) Determinism is proven, not asserted — byte-identical output across four PYTHONHASHSEED values. (5) 8 NON-JAZZ albums (4 Deep Purple, 2 Ulver, 2 Jacqueline du Pre) are in the emitted 300, each carrying only an `artist` reason, because Joseph's real affinity ranks those artists in his top 40 and nothing gates genre. None are in topPicks or any shelf, so the minimal /discover will not show them — it becomes visible the moment a browse-all view ships. Needs a policy call. (6) The 56-item deferred-minors backlog triaged to ZERO must-fix, with 4 items confirmed already fixed by later work.
- **Next**: PLAN 2 — minimal /discover (topPicks + nine shelves, lazy-loaded), then deploy. Resume prompt is at the bottom of this file. Brainstorm the layout with Joseph first; it is deliberately unspecified.
